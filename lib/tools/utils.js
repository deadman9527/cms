...
// 新建JSON文件
Utils.writeJsonFile = function(fileName, destPath, data, isFormat, callback){
	if(isFormat){
	...
	var jsonpDestPath = destPath + 'p';
	var jsonpData = fileName + "("+data+")";
	Utils.writeFile(jsonpDestPath, jsonpData, function(){
		console.log('build ' + jsonpDestPath + ' ok!');
	});
};
